...
export default class NewClass extends cc.Component {
    ...
    cutBtnClick(){
        ...
        // this.cutNode._sgNode.visit();
        let width = this.cutNode.width;
        let height = this.cutNode.height;
        this.cocosBuildImg(width, height, this.cutNode, "photo/battle_result1.png", false, cc.rect(0,0, width, height));


        // this.cocosBuildTest(width, height, this.cutNode, false, cc.rect(0, 0, width, height));
    }
    ...
    cocosBuildImg(imgWidth, imgHeight, buildNode, imgPathAndName, bFormat: boolean = true, rect?: cc.Rect, fullScreen: boolean = true): string {
        ...
        buildNode.scale = 1;
        if (rect) {
            let renderTexture1 = cc.RenderTexture.create(rect.width/2, rect.height/2, cc.Texture2D.PIXEL_FORMAT_RGBA8888, gl.DEPTH24_STENCIL8_OES);
    ...
    cocosBuildTest(imgWidth, imgHeight, buildNode, bFormat: boolean = true, rect?: cc.Rect): string {
        //解析文件名字和路径
        let imgPath = "cut_test/";
        let imgName = "cocosBuildTest.png";
